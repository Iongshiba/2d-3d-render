...
def _build_star() -> TransformNode:
    ...
    pulse = Scale(1.0, 1.0, 1.0, animate=pulse_scale(minimum=0.9, maximum=1.1, speed=2.0))
    star_node = GeometryNode("star_geom", star)
    return TransformNode("star_root", spin, [TransformNode("star_pulse", pulse, [star_node])])


def _build_planet_system(name: str, orbit_radius: float, orbit_speed: float) -> TransformNode:
    ...
    moon_parent = TransformNode(
        f"{name}_moon_orbit",
        Translate(0.0, 0.0, 2.5, animate=circular_orbit(speed=orbit_speed * 2.0, radius=2.5, axis="xz")),
        [GeometryNode(f"{name}_moon", moon_shape)],
    )

    planet_parent = TransformNode(
        f"{name}_planet_root",
        Rotate(axis=(0.0, 1.0, 0.0), angle=0.0, animate=circular_orbit(speed=orbit_speed, radius=orbit_radius, axis="xz")),
        [TransformNode(f"{name}_planet_spin", planet_rotate, [planet_node]), moon_parent],
    )
